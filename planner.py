...
def isYawReached(current, target):
    ...
    return abs(math.cos(current) * math.cos(target) + math.sin(current)*math.sin(target)) < 0.1
...
def Plan(start_point, end_point, drive_path):
    ...
    trace_poses, trace_projection2drive_path_segment_idxs = InferByQuery(vm, drive_path, end_point, kVelocity, kDt)
    
    dubins_start_pose = trace_poses[-1]
    dubins_end_pose = end_point
    dubins_path = Dubins(dubins_start_pose, dubins_end_pose, math.tan(vm.max_delta) / vm.wheel_base, (kVelocity * kDt)/5.0) # 5.0 due to bug in dubins lib
    dubins_path_projection2drive_path_segment_idxs = [-1]*len(dubins_path) # no projection is valid, so all fake as -1
    
    traj = trace_poses + dubins_path
    projection2drive_path_segment_idxs = trace_projection2drive_path_segment_idxs + dubins_path_projection2drive_path_segment_idxs
    for (pose, projection2drive_path_segment_idx)  in zip(traj, projection2drive_path_segment_idxs):
